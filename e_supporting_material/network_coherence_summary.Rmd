...
Shifts in community composition are deemed more significant and globally distributed than changes in species richness. Thus, there's a call to move away from focusing solely on species richness and towards other measures such as network structure and information theory. **Current monitoring approaches often overlook ecological interactions and multi-trophic scenarios, limiting our ability to anticipate biodiversity changes beyond species richness and composition. How can we anticipate changes in those properties?**
...
To do this, we investigate the concept of **Network coherence (CN)**, which
quantifies the degree of association between a community response to a
specific environment and its biotic interactions. High coherence would indicate that interacting species respond similarly and therefore
the community should maintain its internal dynamics and functions; alternatively, incoherence will
induce community re-organization and could lead to disruption of its functioning. **We think NC can therefore become a new indicator to anticipate the consequences of biodiversity changes at the community level.**
...
Our objectives are to **(1) define the Network Coherence concept, its components, and methods to compute it, (2) illustrate the method with data from a real community [ (Pedersen et al. 2017) ](https://royalsocietypublishing.org/doi/full/10.1098/rsos.170215), and (3) mathematically relate NC with ecosystem functioning.**

The study will be expressed in a perspective paper (journal to be determined - e.g. *Ecol Lett, Trends Ecol. Evol.*)
...
$A$ with an environmental coherence matrix $C$ which contains taxa covariances in response to the environment. There are several methods that we could use to express the concordance between the two matrices, and which one we can use is still a matter of debate. One option is the Procrustes analysis, a commonly employed statistical method in various fields, including ecology, to assess the similarity or correspondence between different datasets. It is used to compare two sets of multivariate data by finding an optimal transformation (rotation, translation, scaling, and reflection) that minimizes the differences between them. With this analysis, we are essentially evaluating how well the observed patterns of species interactions match the patterns of covariances in species' responses to environmental variables.

<br>

$$ NC = Procrustes (A, C) $$
> <small>We haven't touched on the different structures that interaction matrices have and how these should be matched with the squared environmental coherence matix</small>
...
regionally from population time series. From time series data, population size can be modeled through time using a hierarchical model that shares information across populations to estimate their joint temporal trends in population size while accounting for their covariation. The correlations between population trends are captured as species’ relationships with latent temporal predictors estimated at each time step, which allow information to be shared across populations at each time step to estimate the average trend. 

<br>

This model can be implemented as a Bayesian dynamic generalized additive model (DGAM), which jointly estimates trends in multiple time series with a dynamic latent component that captures temporal associations between species (Clark & Wells, 2022). It is conceptually analogous to approaches like hierarchical modelling of species communities (HMSC) and joint species distribution models that share information across species to estimate joint responses to unmeasured “latent” predictors (Warton et al., 2015; Thorson et al., 2016; Ovaskainen et al., 2017). 

Species' coefficients $\alpha$ from the model output are their annual rate of change. The correlation of $\alpha$ coefficient between species pairs corresponds to the pairwise co-response to the environment and are used to build the coherence matrix (Hebert et al. unpubl.)


<br>

$$ C_{ij} = cor(\alpha_i, \alpha_j)$$
<br>

<div style="text-align:center">

<img src="supporting_figures/conceptual_NC.png" alt="Figure Caption" width="700">

<br>

 <figcaption style="font-size: smaller;"> Netwok Coherence results from the correlation between the $C$ and $A$ matrices. $C$ matrix of net responses is obtained from species' pairwise correlations in population dynamics - computed with a DGAM model -, and of intrinsic responses from pairwise covariances or correlations of their modeled suitability (computed with SDMs, for a single point or a spatial range in the environment). Interaction data can be computed at local or regional scales depending on the community we consider. </figcaption>
...
-   Interacting species may show higher coherence than non-interacting ones.
...
$$ E[\delta N_{i}] = E[a_{ij}^{(-1)} \cdot E[\delta b_j] + cov(a_{ij}^{(-1)}, \delta b_j) ] $$
...
$$ E[\delta N_{ij}] = E[a_{ij}^{(-1)} \cdot E[\delta b_j] + cov(a_{ij}^{(-1)}, \delta b_j) ] $$
...
<img src="supporting_figures/coherence_mod_nest.jpg" alt="Figure Caption" width="450">
...
<img src="supporting_figures/coherence_niche_breadth.jpg" alt="Figure Caption" width="450">
...
## Activities for the main meeting

<br>

We will work on three different fronts: writing down the concepts, illustrating and describing the behavior of the metric with data from a fish foodweb, and applying it to models for ecosystem functioning and structure.
...
* Definition of NC
* Definition of its components (matrices C and A)
* Formulas of NC and its components
...
> Work with the fish data and compute NC from both net and intrinsic responses.

* Coherence ~ interacting/non-interacting species
* Coherence ~ trophic position
* Coherence ~ generalism level
* Correlation (spp roles in network,  spp roles in environmental coherence)
* Discuss differences in coherence between net/intrinsic interactions
* Weighted vs unweighted A
* Procrustes vs other tests
* NC ~ environmental dimensionality

### APPLICATION

> Develop the maths and simulations to relate NC with models to assess ecosystem functioning and structure.

* Mathematically link NC to ecosystem functioning through LV models.
* Check how NC behaves in LV model.
* Mathematically link NC to beta network diversity.
* Check with simulations whether NC provide information on species roles different than common metrics.
