...
Each correlation has a Beta(a,a) distribution on (-1,1) where a=alphad+(d-2)/2, where d is ghe dimension of the matrix.
...
We therefore choose to run simulations with 8 species. The simulation scheme is as follows:
...
-- A perturbation (delta_r) is computed for each species using `MASS::mvrnorm(n = 1, mu = rep(0, S), Sigma = covMat)`
...
for (alpha_d in c(0.001, 0.1, 1, 10, 20, 40, 60, 100)) {
...
We can still use equation 11 to test how different ENC distributions impact variance in abundance change, simply by inputing different covariance distributions and tunning equation 11 (no numerical analysis).
