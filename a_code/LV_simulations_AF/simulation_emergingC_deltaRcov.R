...
# simulation with random perturbations every n timesteps. I compute the covariance in species' delta_r
...
simulate_with_discrete_perturbations <- function(S, timesteps, cov_matrix, perturb_interval = 100, perturb_scale = 1) {
  ...
  delta_r_matrix <- matrix(0, nrow = timesteps, ncol = S)  # Matrix to store all delta_r over time
  ...
  for (t in seq(perturb_interval, timesteps, by = perturb_interval)) {
    ...
    delta_r_matrix[t, ] <- delta_r  # Store the delta_r values at this timestep
...
for (sim in 1:nsim) {
  ...
  # Compute covariance matrix of the delta_r vectors (instead of biomass changes)
