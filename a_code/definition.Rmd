...
mean_change_strong <- 1.2
mean_change_weak <- 0.5
mean_change_strong_decrease <- -1.2
mean_change_weak_decrease <- -0.5
sd_variation <- 0.1
...
run_simulations_with_metrics <- function(num_simulations, Net_type, S, C, aij_params, scenario, mean_change_strong, mean_change_weak, sd_variation) {
  results <- replicate(num_simulations, {
    ...
    if (scenario == "Strong Positive Increases") {
      rf <- generate_rf_positive(ri, mean_change_strong, sd_variation)
    } else if (scenario == "Weak Positive Increases") {
      rf <- generate_rf_positive(ri, mean_change_weak, sd_variation)
    } else if (scenario == "Strong Positive Decreases") {
      rf <- generate_rf_positive(ri, mean_change_strong_decrease, sd_variation)
    } else if (scenario == "Weak Positive Decreases") {
      rf <- generate_rf_positive(ri, mean_change_weak_decrease, sd_variation)
    } else if (scenario == "Null") {
      rf <- ri
    } else if (scenario == "Mixed Weak") {
      rf <- generate_rf_mixed(ri, mean_change_weak, mean_change_weak_decrease, sd_variation)
    } else if (scenario == "Mixed Strong") {
      rf <- generate_rf_mixed(ri, mean_change_strong, mean_change_strong_decrease, sd_variation)
    }
...
Net_type <- "competition"
C <- 0.2
aij_params <- c(0, 0.1)
num_simulations <- 100

scenarios <- c("Strong Positive Increases", "Weak Positive Increases", "Strong Positive Decreases", "Weak Positive Decreases", "Null", "Mixed Weak", "Mixed Strong")
...
color_palette <- scales::brewer_pal(palette = "RdBu", direction = -1)(7)
mixed_weak_color <- "violet"
mixed_strong_color <- "magenta4"

# Define scenario colors
scenario_colors <- setNames(c(color_palette[1], color_palette[2], color_palette[5], color_palette[1], color_palette[2], mixed_weak_color, mixed_strong_color), scenario_order)
...
#ggsave("fig6.png", height = 8, width = 11)
...
translating to ENC patterns:


```{r}
# Sample one A and C matrix from each scenario
sample_matrices <- combined_matrices_data %>%
  group_by(Scenario) %>%
  sample_n(1)
...
# Initialize an empty list for ENC_matrix
sample_matrices$ENC_matrix <- vector("list", nrow(sample_matrices))

# Calculate ENC_matrix for each row
for (i in 1:nrow(sample_matrices)) {
  sample_matrices$ENC_matrix[[i]] <- calculate_enc_pattern(sample_matrices$A_matrix[[i]], sample_matrices$C_matrix[[i]])
}

# Find overall min and max ENC values across all scenarios
all_enc_values <- unlist(lapply(sample_matrices$ENC_matrix, function(mat) mat[sample_matrices$A_matrix[[1]] != 0]))
min_enc <- min(all_enc_values, na.rm = TRUE)
max_enc <- max(all_enc_values, na.rm = TRUE)

# Create a list to store plots
plots <- list()

# Loop through each scenario and create the plot
for (i in 1:nrow(sample_matrices)) {
  # Sample ENC matrix
  enc_matrix <- sample_matrices$ENC_matrix[[i]]
  
  # Corresponding A matrix
  a_matrix <- sample_matrices$A_matrix[[i]]
  ...
  p <- ggplot(enc_matrix_melt, aes(x = value)) +
    ...
    labs(x = "co-response", y = "Frequency", title = paste("ENC for", sample_matrices$Scenario[i])) +
    theme_classic() +
...
#ggsave("figure8.png", height = 13, width = 9)
...
ggplot(grid, aes(x = influence, y = response, fill = covariance)) +
  geom_tile() +
  scale_fill_distiller(palette = "RdBu", direction = 1, limits = c(-4, 4), name = expression(bar(Delta * X))) +
  ...
  annotate("text", x = -1.5, y = -1.5, label = "(-,-)", size = 6, color = "black") +
  annotate("text", x = 1.5, y = -1.5, label = "(+,-)", size = 6, color = "black") +
  labs(x = "Influence", y = "Response") +
  theme_minimal()

#ggsave("figure9.png", height = 5, width = 6)
...
scaled_influence <- scale_to_range(influence)
scaled_response <- scale_to_range(response)

# Define a grid for influence and response
influence_grid <- seq(-2, 2, length.out = 100)
response_grid <- seq(-2, 2, length.out = 100)

# Create a dataframe with all combinations of influence and response
grid <- expand.grid(influence = influence_grid, response = response_grid)

# Compute the covariance contribution for each combination
grid$covariance <- grid$influence * grid$response

# Convert real network data to a data frame
real_data <- data.frame(influence = scaled_influence, response = scaled_response)
...
#ggsave("figure9.1.png", height = 5, width = 6)
